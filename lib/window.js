...
console.log(paneData);
  ...
  if (paneId) {
    ...
    /*
    console.log(angular.element(sourcePane[0]).scope());
    console.log(sourcePane.scope());
    console.log(sourceContentPane.scope());
    */
    var ctrl = sourcePane.scope();
    console.log(ctrl);
    var pane =  ctrl.panes[0];
    console.log(pane);
    pane.remove();
    //ctrl.vm.removePane(pane);
    return pane;

/*
    newPane = angular.element('<pane caption="' + sourcePane.text() + '"></pane>');
//    console.log(sourceContentPane.contents());
    //sourceContentPane.contents().appendTo(newPane);//.append();
    setTimeout(function() {
      sourcePane.remove();
    }, 0);
	//newPane = sourcePane;
	*/
	return sourceContentPane;
  } else {
...
module.directive('window', function($compile, paneResolverService, $rootScope) {
  return {
    ...
    template: '\
      <div class="window"> \
      	<div class="window-trans-boot" ng-show="false"\
      	             ui-transclude/> \
        <div class="window-header" \
             ui-droppable   \
             ui-drop-accepts="application/x-lx-window-pane"  \
             on-drop="headerDrop(event)"> \
             	<div ng-repeat="pane in panes" \
             	id="{{pane.id}}"\
			    ng-class="{active: pane.active}" \
			      class="tab" \
			      ui-draggable \
			      ui-drag-format="application/x-lx-window-pane" \
			      ui-drag-data=\'{\"id\":\"{{pane.id}}\"}\' \
			      ng-click="pane.select()"> \
             		<span ng-bind="pane.caption">{{pane.active}}</span> \
             	</div> \
             </div>\
    ...
    link: function(scope, element, attr, windowController) {
      ...
      function _drop(event, split) {
        ...
        if (!split) {
/*         	var panepane =  angular.element('<pane caption="Bosse"><b>Test</b></pane>');
          	panepane.appendTo(headerContainer);
			$compile(panepane)(scope.$new(true), undefined, {
				window: windowController
			});*/
			newPane.currentWindow = windowController;
			windowController.addPane(newPane);
        } else {
        	var newWindowController = scope.$new(true);
          var newWindow = $compile('<window></window>')(newWindowController, undefined, {});
/*          newPane.appendTo(newWindow.children('.window-header'));
          $compile(newPane)(scope.$new(true), undefined, {
            window: windowController
          });
*/	
		newWindowController = newWindowController.$$childHead;
		console.log(newWindowController);
		newPane.currentWindow = newWindowController.vm;
		newWindowController.vm.addPane(newPane);
