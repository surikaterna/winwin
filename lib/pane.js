...
module.directive('pane', [ function() {
  return {
    ...
    template: '<div id="{{id}}" \
      ...
      ui-drag-data=\'{\"id\":\"{{id}}\"}\' \
      ng-click="select()"> \
        <span pane-caption-transclude ng-bind="caption"></span> \
    </div>',
    controller: function() {},
    compile: function(element, attrs, transclude) {
      return function postLink(scope, element, attr, windowCtrl) {
        ...
        console.log("::PANE()");
        scope.active = false;
        scope.currentWindow = windowCtrl;
        scope.currentWindow.addPane(scope);
        scope.$watch('active', function(active) {
          if (active) { 
            scope.currentWindow.select(scope); 
          }
        });
...
//          windowCtrl.select(scope);
        ...
        scope.remove = function() {
          scope.currentWindow.removePane(scope);
        }
